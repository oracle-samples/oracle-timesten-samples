...
oci_vcn_ip_routing_del() {
    ...
    if [ -z "$ns" ]; then
        ...
        if $IP rule | grep -qsw $rt_name; then # rule exists
            oci_vcn_debug "rule to table $rt_name delete"
            $IP rule del lookup $rt_name || oci_vcn_warn "cannot delete rule to table $rt_name"
            had_rule='t'
        fi

        # delete default route in table (this would be automatically deleted except for phys ifaces)
        if $IP route show table $rt_name 2>/dev/null | grep -qsw default; then
            oci_vcn_debug "default route in table $rt_name delete"
            $IP route delete default table $rt_name
        fi
...
oci_vcn_config() {
    ...
    # if secondary vnics added and not using namespaces then add src routing to primary
    # (will skip if route table already on primary)
    if [ -n "$found" ] && [ -z "$USE_NS" ]; then
        oci_vcn_ip_routing_add 0 0 "${IP_IFACES[0]}" '' 't'
    fi
...
oci_vcn_deconfig_all() {
    ...
    if [ -z "$found" ]; then
        ...
        # if not using namespaces then remove src routing from primary
        oci_vcn_ip_routing_del 0
    fi
}
