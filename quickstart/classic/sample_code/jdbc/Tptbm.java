		...
		conn.close();
	        ...
	        elapsedTime = controller.getElapsedTime();
                for (tno = 0; tno < numThreads; tno++) {
                    totalXacts += controller.getThread(tno).getTimedXacts();
                }
...
//             "        [-scaleout [local | routing]]\n" +
	...
	System.err.println(
	     ...
	     "                         thread should execute. The default is 10000.\n\n" +
             ...
             "                         Operations in a transaction are randomly chosen\n" +
             "                         between min and max.\n\n" +
	     ...
	     "  -plsql                 Use PL/SQL for -multiop to reduce client-server round trips.\n\n" +
...
/*
	     ...
	     "      local              Constrain all data access to be to rows in the locally\n" +
             "                         connected database element; each thread generates keys\n" +
	     ...
	     "      routing            Use the routing API to optimize data access. Each\n" +
             ...
             "                         or with '-min' or '-max' > 1.\n\n" +
*/

	     "  -trace                 Turns JDBC tracing on.\n\n" +

	     "  -nobuild               Do not build the database; assumes table exists and is\n" +
	     "                         already populated. The '-range' parameter is not relevant.\n\n" +

//	     "  -build                 Builds database and exits. Only the '-key', '-range' and\n" +
//	     "                         '-scaleout' parameters are relevant.\n\n" +
	     "  -build                 Builds database and exits. Only the '-key' and '-range'\n" +
	...
	try {
	    ...
	    pstmt = conn.prepareStatement(insertStmt);
            ...
            // Store the database mode and key valueA
            if (  runMode == modeClassic  )
                dbm = dbModeClassicS;
            else
                dbm = dbModeScaleoutS;
...
*/
	...
	if (nobuild && buildonly) {
	    System.err.println("Please specify only one of -nobuild and -build");
	    System.exit(1);
	}
	...
	if(url == null) {
        ...
        if (  dbms == TPTBM_ORACLE  ) {
            ...
            if (  (dbms == TPTBM_ORACLE) && (runMode > modeClassic)  ) {
                  System.err.println("Scaleout mode can only be used with TimesTen.");
                  System.exit(1);
            }
        }

        if (  !isCS && (runMode == modeScaleoutRouting)  )   {
              System.err.println("Scaleout 'routing' mode can only be used with client/server connections.");
              System.exit(1);
        }
        ...
        if (  runMode > modeScaleout  ) {
              System.err.println("Scaleout 'local' and 'routing' modes cannnot be used with '-multiop'.");
              System.exit(1);
        }
 
        if (  (runMode > modeScaleoutLocal) && ((min_xact != 1) || (max_xact != 1))  ) {
              System.err.println("Scaleout 'routing' mode cannnot be used with '-min' or '-max' > 1.");
              System.exit(1);
        }
